...
//An abstract class that processes input and calculates corresponding Euler angles, Vectors and matrices for use in openGL
class Camera
{
public:
	//Camera attributes
	...
	//processees input received from any keyboard-like input system. Accepts input parameter in the form of camera defined ENUM
	...
	void ProcessMouseMovement(float xoffset, float yoffset, GLboolean constraintPitch = true)
	{
		...
		//Make sure that when the pitch is out of bounds, screen doesnt get flipped
		if (constraintPitch)
		{
			if (Pitch > 89.0f)
				Pitch = 89.0f;
