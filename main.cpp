...
int main()
{
    ...
    // load textures
    // -------------
    uint32_t woodTexture = loadTexture("resources/wood.png");

    //configure floating point framebuffer
    //----------------------------------
    uint32_t hdrfbo;
    glGenFramebuffers(1, &hdrfbo);
    //creating a floating point color buffer
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //2. now render floating point color buffer to 2D quad and tonemap HDR colors to default framebuffers (clamped) color range
        //------------------------------------------------------------------------------------------------------------------------
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        hdrShader.use();
        glActiveTexture(GL_TEXTURE0);
