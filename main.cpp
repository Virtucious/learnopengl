...
uint32_t loadTexure(char const* path);
...
int main()
{
    ...
    //set up the vertex data of the plane
    //----------------------------------
    float planeVertices[] = {
    ...
    //load textures
    //-------------
    uint32_t floorTexture = loadTexure("resources/woodenFloor.png");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //1. render depth of scene to the texture(from light's perspective)
...
void processInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_B) == GLFW_PRESS && !blinn_is_pressed)
    {
        blinn = !blinn;
        blinn_is_pressed = true;
    }
    if (glfwGetKey(window, GLFW_KEY_B) == GLFW_RELEASE)
    {
        blinn_is_pressed = false;
    }
}
...
uint32_t loadTexure(char const* path)
{
    ...
    {
        std::cout << "Texture failed to load at point: " << path <<std::endl;
        stbi_image_free(data);
    }
